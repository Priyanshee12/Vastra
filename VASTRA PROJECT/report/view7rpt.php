...
<?php include_once "view7rptinfo.php" ?>
<?php
...
$view7_rpt = NULL; // Initialize page object first

class crview7_rpt extends crview7 {
	...
	function __construct() {
		...
		// Table object (view7)
		if (!isset($GLOBALS["view7"])) {
			$GLOBALS["view7"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["view7"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'view7', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fview7rpt";
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_view7\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_view7',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fview7rpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fview7rpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fview7rpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function GetRow($opt) {
		...
		if (!$rs->EOF) {
			...
			$this->Val[1] = $this->category_name->CurrentValue;
		} else {
...
// Create page object
if (!isset($view7_rpt)) $view7_rpt = new crview7_rpt();
...
// Create page object
var view7_rpt = new ewr_Page("view7_rpt");

// Page properties
view7_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = view7_rpt.PageID;
</script>
<?php } ?>
...
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_view7" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="category_name"><div class="view7_category_name"><span class="ewTableHeaderCaption"><?php echo $Page->category_name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="category_name">
<?php if ($Page->SortUrl($Page->category_name) == "") { ?>
		<div class="ewTableHeaderBtn view7_category_name">
			<span class="ewTableHeaderCaption"><?php echo $Page->category_name->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer view7_category_name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->category_name) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->category_name->FldCaption() ?></span>
