...
//		$item->Visible = TRUE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['product_price'] = ewr_Conv($rs->fields('product_price'), 3);
				$this->FirstRowData['product_size'] = ewr_Conv($rs->fields('product_size'), 200);
		...
		if (!$rs->EOF) {
			...
			$this->product_size->setDbValue($rs->fields('product_size'));
			$this->image->setDbValue($rs->fields('image'));
			$this->quantity->setDbValue($rs->fields('quantity'));
			$this->description->setDbValue($rs->fields('description'));
			$this->category_name->setDbValue($rs->fields('category_name'));
			$this->Val[1] = $this->product_name->CurrentValue;
			$this->Val[2] = $this->product_price->CurrentValue;
			$this->Val[3] = $this->product_size->CurrentValue;
			$this->Val[4] = $this->image->CurrentValue;
			$this->Val[5] = $this->quantity->CurrentValue;
			$this->Val[6] = $this->category_name->CurrentValue;
		} else {
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			// Check popup name
			// Build distinct values for category_name

			if ($popupname == 'view1_category_name') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;

				// Call Page Filtering event
				$this->Page_Filtering($this->category_name, $sFilter, "popup");
				$sSql = ewr_BuildReportSql($this->category_name->SqlSelect, $this->getSqlWhere(), $this->getSqlGroupBy(), $this->getSqlHaving(), $this->category_name->SqlOrderBy, $sFilter, "");
				$rswrk = $conn->Execute($sSql);
				while ($rswrk && !$rswrk->EOF) {
					$this->category_name->setDbValue($rswrk->fields[0]);
					$this->category_name->ViewValue = @$rswrk->fields[1];
					if (is_null($this->category_name->CurrentValue)) {
						$bNullValue = TRUE;
					} elseif ($this->category_name->CurrentValue == "") {
						$bEmptyValue = TRUE;
					} else {
						ewr_SetupDistinctValues($this->category_name->ValueList, $this->category_name->CurrentValue, $this->category_name->ViewValue, FALSE, $this->category_name->FldDelimiter);
					}
					$rswrk->MoveNext();
				}
				if ($rswrk)
					$rswrk->Close();
				if ($bEmptyValue)
					ewr_SetupDistinctValues($this->category_name->ValueList, EWR_EMPTY_VALUE, $ReportLanguage->Phrase("EmptyLabel"), FALSE);
				if ($bNullValue)
					ewr_SetupDistinctValues($this->category_name->ValueList, EWR_NULL_VALUE, $ReportLanguage->Phrase("NullLabel"), FALSE);
				$fld = &$this->category_name;
			}
	...
	function SetupPopup() {
		...
		if (is_array(@$_SESSION["sel_view1_category_name"])) {
			$this->LoadSelectionFromSession('category_name');
		} elseif (@$_SESSION["sel_view1_category_name"] == EWR_INIT_VALUE) { // Select all
			$this->category_name->SelectionList = "";
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			$this->product_size->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
	...
	function SetupFieldCount() {
		...
		if ($this->product_size->Visible) $this->DtlColumnCount += 1;
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->PopupName == 'view1_category_name' && $this->PopupValue <> "") {
				if ($this->PopupValue == EWR_INIT_VALUE)
					$this->category_name->DropDownValue = EWR_ALL_VALUE;
				else
					$this->category_name->DropDownValue = $this->PopupValue;
				$bRestoreSession = FALSE; // Do not restore
			} elseif ($this->ClearExtFilter == 'view1_category_name') {
				$this->SetSessionDropDownValue(EWR_INIT_VALUE, '', 'category_name');
			}
		...
		// Reset search command
			...
			// Load default values
			$this->SetSessionDropDownValue($this->category_name->DropDownValue, $this->category_name->SearchOperator, 'category_name'); // Field category_name
			...
			if ($this->GetDropDownValue($this->category_name)) {
				$bSetupFilter = TRUE;
			} elseif ($this->category_name->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_view1_category_name'])) {
				$bSetupFilter = TRUE;
			}
		...
		// Build SQL
		$this->BuildDropDownFilter($this->category_name, $sFilter, $this->category_name->SearchOperator, FALSE, TRUE); // Field category_name
		...
		if ($bSetupFilter) {
			...
			// Field category_name
			$sWrk = "";
			$this->BuildDropDownFilter($this->category_name, $sWrk, $this->category_name->SearchOperator);
			ewr_LoadSelectionFromFilter($this->category_name, $sWrk, $this->category_name->SelectionList, $this->category_name->DropDownValue);
			$_SESSION['sel_view1_category_name'] = ($this->category_name->SelectionList == "") ? EWR_INIT_VALUE : $this->category_name->SelectionList;
		}

		// Field category_name
		ewr_LoadDropDownList($this->category_name->DropDownList, $this->category_name->DropDownValue);
		return $sFilter;
	}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field category_name
		$this->category_name->DefaultDropDownValue = EWR_INIT_VALUE;
		if (!$this->SearchCommand) $this->category_name->DropDownValue = $this->category_name->DefaultDropDownValue;
		$sWrk = "";
		$this->BuildDropDownFilter($this->category_name, $sWrk, $this->category_name->SearchOperator, TRUE);
		ewr_LoadSelectionFromFilter($this->category_name, $sWrk, $this->category_name->DefaultSelectionList);
		if (!$this->SearchCommand) $this->category_name->SelectionList = $this->category_name->DefaultSelectionList;
		/**
		...
		*/
		...
		// Field category_name
		// $this->category_name->DefaultSelectionList = array("val1", "val2");
	...
	function CheckFilter() {
		...
		if (!ewr_MatchedArray($this->category_name->DefaultSelectionList, $this->category_name->SelectionList))
			return TRUE;
		return FALSE;
	}
	...
	function ShowFilterList($showDate = FALSE) {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildDropDownFilter($this->category_name, $sExtWrk, $this->category_name->SearchOperator);
		if (is_array($this->category_name->SelectionList))
			$sWrk = ewr_JoinArray($this->category_name->SelectionList, ", ", EWR_DATATYPE_STRING, 0, $this->DBID);
		$sFilter = "";
		if ($sExtWrk <> "")
		...
		if ($sFilter <> "")
			$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $this->category_name->FldCaption() . "</span>" . $sFilter . "</div>";
		$divstyle = "";
		$divdataclass = "";
	...
	function GetFilterList() {
		...
		// Field category_name
		$sWrk = "";
		$sWrk = ($this->category_name->DropDownValue <> EWR_INIT_VALUE) ? $this->category_name->DropDownValue : "";
		if (is_array($sWrk))
			$sWrk = implode("||", $sWrk);
		if ($sWrk <> "")
			$sWrk = "\"sv_category_name\":\"" . ewr_JsEncode2($sWrk) . "\"";
		if ($sWrk == "") {
			$sWrk = ($this->category_name->SelectionList <> EWR_INIT_VALUE) ? $this->category_name->SelectionList : "";
			if (is_array($sWrk))
				$sWrk = implode("||", $sWrk);
			if ($sWrk <> "")
				$sWrk = "\"sel_category_name\":\"" . ewr_JsEncode2($sWrk) . "\"";
		}
		if ($sWrk <> "") {
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sv_category_name", $filter)) {
			$sWrk = $filter["sv_category_name"];
			if (strpos($sWrk, "||") !== FALSE)
				$sWrk = explode("||", $sWrk);
			$this->SetSessionDropDownValue($sWrk, @$filter["so_category_name"], "category_name");
			$bRestoreFilter = TRUE;
		}
		if (array_key_exists("sel_category_name", $filter)) {
			$sWrk = $filter["sel_category_name"];
			$sWrk = explode("||", $sWrk);
			$this->category_name->SelectionList = $sWrk;
			$_SESSION["sel_view1_category_name"] = $sWrk;
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "category_name"); // Clear drop down
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "category_name");
			$this->category_name->SelectionList = "";
			$_SESSION["sel_view1_category_name"] = "";
		}
		return TRUE;
	}
	...
	function GetPopupFilter() {
		...
		if ($this->DrillDown)
			return "";
		if (!$this->DropDownFilterExist($this->category_name, $this->category_name->SearchOperator)) {
			if (is_array($this->category_name->SelectionList)) {
				$sFilter = ewr_FilterSql($this->category_name, "`category_name`", EWR_DATATYPE_STRING, $this->DBID);

				// Call Page Filtering event
				$this->Page_Filtering($this->category_name, $sFilter, "popup");
				$this->category_name->CurrentFilter = $sFilter;
...
// Use Ajax
fview1rpt.Lists["sv_category_name"] = {"LinkField":"sv_category_name","Ajax":true,"DisplayFields":["sv_category_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown && !$Page->ShowReportInDashboard) { ?>
<script type="text/javascript">
...
<div id="c_category_name" class="ewCell form-group">
	<label for="sv_category_name" class="ewSearchCaption ewLabel"><?php echo $Page->category_name->FldCaption() ?></label>
	<span class="ewSearchField">
<?php $Page->category_name->EditAttrs["onchange"] = "ewrForms(this).Submit(); " . @$Page->category_name->EditAttrs["onchange"]; ?>
<?php ewr_PrependClass($Page->category_name->EditAttrs["class"], "form-control"); ?>
<select data-table="view1" data-field="x_category_name" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->category_name->DisplayValueSeparator) ? json_encode($Page->category_name->DisplayValueSeparator) : $Page->category_name->DisplayValueSeparator) ?>" id="sv_category_name" name="sv_category_name"<?php echo $Page->category_name->EditAttributes() ?>>
<option value=""><?php echo $ReportLanguage->Phrase("PleaseSelect") ?></option>
<?php
	$cntf = is_array($Page->category_name->AdvancedFilters) ? count($Page->category_name->AdvancedFilters) : 0;
	$cntd = is_array($Page->category_name->DropDownList) ? count($Page->category_name->DropDownList) : 0;
	$totcnt = $cntf + $cntd;
	$wrkcnt = 0;
	if ($cntf > 0) {
		foreach ($Page->category_name->AdvancedFilters as $filter) {
			if ($filter->Enabled) {
				$selwrk = ewr_MatchedFilterValue($Page->category_name->DropDownValue, $filter->ID) ? " selected" : "";
?>
...
<?php
	...
	for ($i = 0; $i < $cntd; $i++) {
		$selwrk = " selected";
?>
<option value="<?php echo $Page->category_name->DropDownList[$i] ?>"<?php echo $selwrk ?>><?php echo ewr_DropDownDisplayValue($Page->category_name->DropDownList[$i], "", 0) ?></option>
<?php
		$wrkcnt += 1;
	}
?>
</select>
<input type="hidden" name="s_sv_category_name" id="s_sv_category_name" value="<?php echo $Page->category_name->LookupFilterQuery() ?>">
<script type="text/javascript">
fview1rpt.Lists["sv_category_name"].Options = <?php echo ewr_ArrayToJson($Page->category_name->LookupFilterOptions) ?>;
</script>
</span>
</div>
</div>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="product_size"><div class="view1_product_size"><span class="ewTableHeaderCaption"><?php echo $Page->product_size->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="product_size">
<?php if ($Page->SortUrl($Page->product_size) == "") { ?>
		<div class="ewTableHeaderBtn view1_product_size">
			<span class="ewTableHeaderCaption"><?php echo $Page->product_size->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer view1_product_size" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->product_size) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->product_size->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->product_size->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->product_size->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="image"><div class="view1_image"><span class="ewTableHeaderCaption"><?php echo $Page->image->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="image">
<?php if ($Page->SortUrl($Page->image) == "") { ?>
		<div class="ewTableHeaderBtn view1_image">
			<span class="ewTableHeaderCaption"><?php echo $Page->image->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer view1_image" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->image) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->image->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="quantity"><div class="view1_quantity"><span class="ewTableHeaderCaption"><?php echo $Page->quantity->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="quantity">
<?php if ($Page->SortUrl($Page->quantity) == "") { ?>
		<div class="ewTableHeaderBtn view1_quantity">
			<span class="ewTableHeaderCaption"><?php echo $Page->quantity->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer view1_quantity" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->quantity) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->quantity->FldCaption() ?></span>
...
<?php if ($Page->SortUrl($Page->category_name) == "") { ?>
	...
	<?php if (!$Page->ShowReportInDashboard) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'view1_category_name', range: false, from: '<?php echo $Page->category_name->RangeFrom; ?>', to: '<?php echo $Page->category_name->RangeTo; ?>', url: 'view1rpt.php' });" id="x_category_name<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
...
<span<?php echo $Page->product_price->ViewAttributes() ?>><?php echo $Page->product_price->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->product_size->Visible) { ?>
		<td data-field="product_size"<?php echo $Page->product_size->CellAttributes() ?>>
<span<?php echo $Page->product_size->ViewAttributes() ?>><?php echo $Page->product_size->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->image->Visible) { ?>
...
<span<?php echo $Page->category_name->ViewAttributes() ?>><?php echo $Page->category_name->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
