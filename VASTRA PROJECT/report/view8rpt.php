...
<?php include_once "view8rptinfo.php" ?>
<?php
...
$view8_rpt = NULL; // Initialize page object first

class crview8_rpt extends crview8 {
	...
	function __construct() {
		...
		// Table object (view8)
		if (!isset($GLOBALS["view8"])) {
			$GLOBALS["view8"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["view8"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'view8', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fview8rpt";
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_view8\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_view8',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fview8rpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fview8rpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fview8rpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function Page_Main() {
		...
		$this->rawmaterial_name->SetVisibility();
		$this->r_detail->SetVisibility();
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['rawmaterial_name'] = ewr_Conv($rs->fields('rawmaterial_name'), 200);
				$this->FirstRowData['r_detail'] = ewr_Conv($rs->fields('r_detail'), 200);
				$this->FirstRowData['QTY'] = ewr_Conv($rs->fields('QTY'), 3);
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
			$this->rawmaterial_name->setDbValue($rs->fields('rawmaterial_name'));
			$this->r_detail->setDbValue($rs->fields('r_detail'));
			$this->QTY->setDbValue($rs->fields('QTY'));
			$this->Val[1] = $this->rawmaterial_name->CurrentValue;
			$this->Val[2] = $this->r_detail->CurrentValue;
			$this->Val[3] = $this->QTY->CurrentValue;
		} else {
			$this->rawmaterial_name->setDbValue("");
			$this->r_detail->setDbValue("");
			$this->QTY->setDbValue("");
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			// rawmaterial_name
			$this->rawmaterial_name->HrefValue = "";

			// r_detail
			$this->r_detail->HrefValue = "";
			...
			// rawmaterial_name
			$this->rawmaterial_name->ViewValue = $this->rawmaterial_name->CurrentValue;
			$this->rawmaterial_name->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";

			// r_detail
			$this->r_detail->ViewValue = $this->r_detail->CurrentValue;
			$this->r_detail->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// rawmaterial_name
			$CurrentValue = $this->rawmaterial_name->CurrentValue;
			$ViewValue = &$this->rawmaterial_name->ViewValue;
			$ViewAttrs = &$this->rawmaterial_name->ViewAttrs;
			$CellAttrs = &$this->rawmaterial_name->CellAttrs;
			$HrefValue = &$this->rawmaterial_name->HrefValue;
			$LinkAttrs = &$this->rawmaterial_name->LinkAttrs;
			$this->Cell_Rendered($this->rawmaterial_name, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);

			// r_detail
			$CurrentValue = $this->r_detail->CurrentValue;
			$ViewValue = &$this->r_detail->ViewValue;
			$ViewAttrs = &$this->r_detail->ViewAttrs;
			$CellAttrs = &$this->r_detail->CellAttrs;
			$HrefValue = &$this->r_detail->HrefValue;
			$LinkAttrs = &$this->r_detail->LinkAttrs;
			$this->Cell_Rendered($this->r_detail, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
	...
	function SetupFieldCount() {
		...
		if ($this->rawmaterial_name->Visible) $this->DtlColumnCount += 1;
		if ($this->r_detail->Visible) $this->DtlColumnCount += 1;
	...
	function GetSort($options = array()) {
		...
		if ($bResetSort) {
			...
			$this->rawmaterial_name->setSort("");
			$this->r_detail->setSort("");
			$this->QTY->setSort("");
...
// Create page object
if (!isset($view8_rpt)) $view8_rpt = new crview8_rpt();
...
// Create page object
var view8_rpt = new ewr_Page("view8_rpt");

// Page properties
view8_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = view8_rpt.PageID;
</script>
<?php } ?>
...
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_view8" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
<table class="<?php echo $Page->ReportTableClass ?>">
<thead>
	<!-- Table header -->
	<tr class="ewTableHeader">
<?php if ($Page->rawmaterial_name->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="rawmaterial_name"><div class="view8_rawmaterial_name"><span class="ewTableHeaderCaption"><?php echo $Page->rawmaterial_name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="rawmaterial_name">
<?php if ($Page->SortUrl($Page->rawmaterial_name) == "") { ?>
		<div class="ewTableHeaderBtn view8_rawmaterial_name">
			<span class="ewTableHeaderCaption"><?php echo $Page->rawmaterial_name->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer view8_rawmaterial_name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->rawmaterial_name) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->rawmaterial_name->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->rawmaterial_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->rawmaterial_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
<?php } ?>
<?php if ($Page->r_detail->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="r_detail"><div class="view8_r_detail"><span class="ewTableHeaderCaption"><?php echo $Page->r_detail->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="r_detail">
<?php if ($Page->SortUrl($Page->r_detail) == "") { ?>
		<div class="ewTableHeaderBtn view8_r_detail">
			<span class="ewTableHeaderCaption"><?php echo $Page->r_detail->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer view8_r_detail" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->r_detail) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->r_detail->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->r_detail->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->r_detail->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="QTY"><div class="view8_QTY"><span class="ewTableHeaderCaption"><?php echo $Page->QTY->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="QTY">
<?php if ($Page->SortUrl($Page->QTY) == "") { ?>
		<div class="ewTableHeaderBtn view8_QTY">
			<span class="ewTableHeaderCaption"><?php echo $Page->QTY->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer view8_QTY" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->QTY) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->QTY->FldCaption() ?></span>
...
<?php if ($Page->rawmaterial_name->Visible) { ?>
		<td data-field="rawmaterial_name"<?php echo $Page->rawmaterial_name->CellAttributes() ?>>
<span<?php echo $Page->rawmaterial_name->ViewAttributes() ?>><?php echo $Page->rawmaterial_name->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->r_detail->Visible) { ?>
		<td data-field="r_detail"<?php echo $Page->r_detail->CellAttributes() ?>>
<span<?php echo $Page->r_detail->ViewAttributes() ?>><?php echo $Page->r_detail->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->QTY->Visible) { ?>
