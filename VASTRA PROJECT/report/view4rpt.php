...
//		$item->Visible = TRUE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['product_id'] = ewr_Conv($rs->fields('product_id'), 3);
		...
		if (!$rs->EOF) {
			...
			$this->Val[1] = $this->product_id->CurrentValue;
			$this->Val[2] = $this->user_id->CurrentValue;
		} else {
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			$this->product_id->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
	...
	function SetupFieldCount() {
		...
		if ($this->product_id->Visible) $this->DtlColumnCount += 1;
...
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_view4" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="product_id"><div class="view4_product_id"><span class="ewTableHeaderCaption"><?php echo $Page->product_id->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="product_id">
<?php if ($Page->SortUrl($Page->product_id) == "") { ?>
		<div class="ewTableHeaderBtn view4_product_id">
			<span class="ewTableHeaderCaption"><?php echo $Page->product_id->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer view4_product_id" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->product_id) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->product_id->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->product_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->product_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<span<?php echo $Page->product_id->ViewAttributes() ?>><?php echo $Page->product_id->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->user_id->Visible) { ?>
		<td data-field="user_id"<?php echo $Page->user_id->CellAttributes() ?>>
<span<?php echo $Page->user_id->ViewAttributes() ?>><?php echo $Page->user_id->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
