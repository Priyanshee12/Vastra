...
//		$item->Visible = TRUE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['subcategory_name'] = ewr_Conv($rs->fields('subcategory_name'), 200);
		...
		if (!$rs->EOF) {
			...
			$this->Val[1] = $this->subcategory_name->CurrentValue;
			$this->Val[2] = $this->category_name->CurrentValue;
		} else {
			$this->subcategory_name->setDbValue("");
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'view2_category_name') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
	...
	function SetupPopup() {
		...
		if (is_array(@$_SESSION["sel_view2_category_name"])) {
			$this->LoadSelectionFromSession('category_name');
		} elseif (@$_SESSION["sel_view2_category_name"] == EWR_INIT_VALUE) { // Select all
			$this->category_name->SelectionList = "";
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			$this->subcategory_name->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			$this->Cell_Rendered($this->subcategory_name, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
	...
	function SetupFieldCount() {
		...
		if ($this->subcategory_name->Visible) $this->DtlColumnCount += 1;
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->PopupName == 'view2_category_name' && $this->PopupValue <> "") {
			...
			} elseif ($this->ClearExtFilter == 'view2_category_name') {
		...
		// Reset search command
			...
			if ($this->GetDropDownValue($this->category_name)) {
				$bSetupFilter = TRUE;
			} elseif ($this->category_name->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_view2_category_name'])) {
				$bSetupFilter = TRUE;
			}
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_view2_category_name'] = ($this->category_name->SelectionList == "") ? EWR_INIT_VALUE : $this->category_name->SelectionList;
		}
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sel_category_name", $filter)) {
			...
			$_SESSION["sel_view2_category_name"] = $sWrk;
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "category_name");
			$this->category_name->SelectionList = "";
			$_SESSION["sel_view2_category_name"] = "";
		}
		return TRUE;
	}
...
// Form object
var CurrentForm = fview2rpt = new ewr_Form("fview2rpt");
...
// Use Ajax
fview2rpt.Lists["sv_category_name"] = {"LinkField":"sv_category_name","Ajax":true,"DisplayFields":["sv_category_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown && !$Page->ShowReportInDashboard) { ?>
<script type="text/javascript">
...
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_sv_category_name"><?php echo $ReportLanguage->Phrase("PleaseSelect") ?></span>
</span>
<button type="button" title="<?php echo ewr_HtmlEncode(str_replace("%s", ewr_RemoveHtml($Page->category_name->FldCaption()), $ReportLanguage->Phrase("LookupLink", TRUE))) ?>" onclick="ewr_ModalLookupShow({lnk:this,el:'sv_category_name',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="view2" data-field="x_category_name" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $Page->category_name->DisplayValueSeparatorAttribute() ?>" name="sv_category_name" id="sv_category_name" value="<?php echo ewr_FilterCurrentValue($Page->category_name, ",") ?>"<?php echo $Page->category_name->EditAttributes() ?>>
<input type="hidden" name="s_sv_category_name" id="s_sv_category_name" value="<?php echo $Page->category_name->LookupFilterQuery() ?>">
<script type="text/javascript">
fview2rpt.Lists["sv_category_name"].Options = <?php echo ewr_ArrayToJson($Page->category_name->LookupFilterOptions) ?>;
</script>
</span>
</div>
</div>
...
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_view2" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="subcategory_name"><div class="view2_subcategory_name"><span class="ewTableHeaderCaption"><?php echo $Page->subcategory_name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="subcategory_name">
<?php if ($Page->SortUrl($Page->subcategory_name) == "") { ?>
		<div class="ewTableHeaderBtn view2_subcategory_name">
			<span class="ewTableHeaderCaption"><?php echo $Page->subcategory_name->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer view2_subcategory_name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->subcategory_name) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->subcategory_name->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->subcategory_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->subcategory_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="category_name"><div class="view2_category_name"><span class="ewTableHeaderCaption"><?php echo $Page->category_name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="category_name">
<?php if ($Page->SortUrl($Page->category_name) == "") { ?>
		<div class="ewTableHeaderBtn view2_category_name">
			<span class="ewTableHeaderCaption"><?php echo $Page->category_name->FldCaption() ?></span>
	<?php if (!$Page->ShowReportInDashboard) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'view2_category_name', range: false, from: '<?php echo $Page->category_name->RangeFrom; ?>', to: '<?php echo $Page->category_name->RangeTo; ?>', url: 'view2rpt.php' });" id="x_category_name<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer view2_category_name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->category_name) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->category_name->FldCaption() ?></span>
...
<?php if ($Page->subcategory_name->Visible) { ?>
		<td data-field="subcategory_name"<?php echo $Page->subcategory_name->CellAttributes() ?>>
<span<?php echo $Page->subcategory_name->ViewAttributes() ?>><?php echo $Page->subcategory_name->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->category_name->Visible) { ?>
